use api::{not_found_response, unauthorized_response, ApiResponse};
...
async fn main() -> Result<(), Error> {
    ...
    let handler = move |event: Request| me(event, db_context_ref);
...
async fn me(event: Request, db_context_ref: &PgPool) -> Result<impl IntoResponse, Error> {
    ...
    if let Some(header_value) = auth_header {
        ...
        let user = Users::get_user_by_oauth_id(db_context_ref, oauth_id)
            .await
            .unwrap();
